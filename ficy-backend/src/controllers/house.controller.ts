...
export const getById = async (req: Request, res: Response): Promise<any> => {
  ...
  try {
    const book = await houseService.getHouseByUrl(parse.data.id);
    res
      .status(200)
...
export const getByIds = async (req: Request, res: Response): Promise<any> => {
  ...
  try {
    const book = await houseService.getHouseByUrls(parse.data);
    res
      .status(200)
      .json(buildResponseJson({ status: 200, message: "Success", data: book }));
  } catch (e: any) {
...
export const getPaginated = async (
  ...
  const data = await houseService.getHousesPaginated(
    Number(page),
    Number(limit)
  );
  res
...
export const search = async (req: Request, res: Response): Promise<any> => {
  ...
  const data = await houseService.searchHouses(parse.data.keyword);
  res
    .status(200)
    .json(buildResponseJson({ status: 200, message: "Success", data }));
};
